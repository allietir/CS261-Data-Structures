...
/* Name: Alexander Tir
* Date: 2/22/2018
* Solution description: Heap-based priority queue implementation using dynamic array
*/
#include <assert.h>
#include <stdlib.h>
#include "dynArray.h"
...
void addHeap(DynArr *heap, TYPE node)
{
  	...
  	int parent;
  	int position = sizeDynArr(heap) - 1;

  	while(position != 0)
  	{
  		parent = (position - 1)/2;
  		if(heap->data[parent].priority > heap->data[position].priority)
  		{
  			swapDynArr(heap, position, parent);
  			position = parent;
  		}
  		else
  			return;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
  	...
  	if(rightChild < max)
  	{
  		...
  		if(heap->data[pos].priority > heap->data[smallestChild].priority)
  		{
  			swapDynArr(heap, pos, smallestChild);
  			_adjustHeap(heap, max, smallestChild);
  		}
  	}
  	else if(leftChild < max) //there is only one child
  	{
  		if(heap->data[pos].priority > heap->data[leftChild].priority)
  		{
